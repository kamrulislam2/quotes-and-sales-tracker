...
export const AdminCustomEntryModal: React.FC<AdminCustomEntryModalProps> = ({
  ...
  const [adminCustomFileType, setAdminCustomFileType] = useState<FileType>('Quote');
  ...
  useEffect(() => {
    if (adminCustomUserId && targetAllowedCategories.length > 0) {
      if (!targetAllowedCategories.includes(adminCustomFileType)) {
        ...
        } else if (adminCustomFileType.startsWith('Review') && isReviewAllowed) {
          // Keep review
        } else {
  ...
  }, [adminCustomUserId, targetAllowedCategories, adminCustomFileType, isRequoteAllowed, isReviewAllowed]);
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setAdminCustomBranchName(e.target.value.toUpperCase())}
                className="block w-full px-3.5 py-2.5 bg-slate-955 border border-slate-800 rounded-xl text-white placeholder-slate-650 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition-all"
              />
            </div>
          </div>
          ...
          <div className="space-y-4 flex flex-col justify-between">
            ...
            <div className="flex gap-3 pt-4 border-t border-slate-800">
              ...
              >
                {submitting ? (
                  <>
                    <Loader2 className="animate-spin h-3.5 w-3.5 text-white" /> Submitting...
                  </>
                ) : (
                  <>
